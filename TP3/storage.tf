...
# Création du container pour le storage
...
data "azurerm_virtual_machine" "myVirtualMachine" {
  name                = azurerm_linux_virtual_machine.myLinuxVirtualMachine_2.name
  resource_group_name = azurerm_resource_group.myRessourceGroup.name
}

# Assignation du rôle blob contributor pour le storage
...
resource "azurerm_role_assignment" "vm_blob_access" {
  ...
  depends_on = [
    azurerm_linux_virtual_machine.myLinuxVirtualMachine_2
  ]
}